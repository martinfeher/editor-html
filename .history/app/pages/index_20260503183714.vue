...
/** Print card size (mm); matches PDF export — used for sidebar mm fields. */
const CARD_MM_W = 88.9
const CARD_MM_H = 50.8
...
function parseTextBlock(raw: Record<string, unknown>, id: string): TextBlock | null {
  ...
  const lh = Number.isNaN(lineHeight) ? 1.25 : Math.min(3, Math.max(0.8, lineHeight))
  return {
    kind: 'text',
    id,
    content: raw.content,
...
function selectedLayerIndex(): number {
  ...
  return activeBlocks().findIndex((b) => b.id === selectedId.value)
}

function moveLayerToFront() {
  const arr = activeBlocks()
...
function moveLayerForward() {
  const i = selectedLayerIndex()
  if (i === -1) return
  moveLayerAt(i, 1)
}

function moveLayerBackward() {
  const i = selectedLayerIndex()
  if (i === -1) return
  moveLayerAt(i, -1)
}
...
function duplicateSelected() {
  ...
  if (sel.kind === 'text') {
    const copy: TextBlock = {
      ...sel,
      id: newId(),
      x: Math.round(Math.min(92, sel.x + 2) * 10) / 10,
      y: Math.round(Math.min(92, sel.y + 2) * 10) / 10,
    }
...
function pctToMmW(pct: number) {
  return Math.round((pct / 100) * CARD_MM_W * 100) / 100
}

function pctToMmH(pct: number) {
  return Math.round((pct / 100) * CARD_MM_H * 100) / 100
}

function mmToPctW(mm: number) {
  return (mm / CARD_MM_W) * 100
}

function mmToPctH(mm: number) {
  return (mm / CARD_MM_H) * 100
}

function clampDimPct(v: number) {
  return Math.min(100, Math.max(MIN_BOX_PCT, Math.round(v * 10) / 10))
}

function parseMmInput(raw: string) {
  const n = Number(String(raw).replace(',', '.'))
  return Number.isFinite(n) ? n : NaN
}

const aspectRatioLock = ref(false)
const lockedAspectWH = ref(1)

watch(selectedId, () => {
  aspectRatioLock.value = false
})

function toggleAspectRatioLock() {
  const b = selected.value
  if (!b) return
  if (aspectRatioLock.value) {
    aspectRatioLock.value = false
    return
  }
  lockedAspectWH.value = b.widthPct / Math.max(MIN_BOX_PCT / 2, b.heightPct)
  aspectRatioLock.value = true
}

function onMmWidthInput(raw: string) {
  const b = selected.value
  if (!b) return
  const mm = parseMmInput(raw)
  if (Number.isNaN(mm)) return
  const pct = clampDimPct(mmToPctW(mm))
  b.widthPct = pct
  if (aspectRatioLock.value) {
    b.heightPct = clampDimPct(pct / lockedAspectWH.value)
  }
}

function onMmHeightInput(raw: string) {
  const b = selected.value
  if (!b) return
  const mm = parseMmInput(raw)
  if (Number.isNaN(mm)) return
  const pct = clampDimPct(mmToPctH(mm))
  b.heightPct = pct
  if (aspectRatioLock.value) {
    b.widthPct = clampDimPct(pct * lockedAspectWH.value)
  }
}

function onMmLeftInput(raw: string) {
  const b = selected.value
  if (!b) return
  const mm = parseMmInput(raw)
  if (Number.isNaN(mm)) return
  const pct = (mm / CARD_MM_W) * 100
  const maxX = Math.max(0, 100 - b.widthPct)
  b.x = Math.min(maxX, Math.max(0, Math.round(pct * 10) / 10))
}

function onMmTopInput(raw: string) {
  const b = selected.value
  if (!b) return
  const mm = parseMmInput(raw)
  if (Number.isNaN(mm)) return
  const pct = (mm / CARD_MM_H) * 100
  const maxY = Math.max(0, 100 - b.heightPct)
  b.y = Math.min(maxY, Math.max(0, Math.round(pct * 10) / 10))
}

function alignSelectedGrid(row: number, col: number) {
  const b = selected.value
  if (!b) return
  const v = row === 0 ? 'top' : row === 1 ? 'middle' : 'bottom'
  const h = col === 0 ? 'left' : col === 1 ? 'center' : 'right'
  if (v === 'top') b.y = 0
  else if (v === 'middle') b.y = Math.round(((100 - b.heightPct) / 2) * 10) / 10
  else b.y = Math.round((100 - b.heightPct) * 10) / 10
  if (h === 'left') b.x = 0
  else if (h === 'center') b.x = Math.round(((100 - b.widthPct) / 2) * 10) / 10
  else b.x = Math.round((100 - b.widthPct) * 10) / 10
}

function fontSizePxToMmApprox(px: number) {
...
async function exportCardToPdf() {
  ...
  try {
    ...
    pdf.addImage(imgData, 'PNG', 0, 0, CARD_MM_W, CARD_MM_H, undefined, 'SLOW')
    const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')
...
<template>
  <div class="editor-root">
    ...
    <aside class="sidebar" aria-label="Tools and properties">
      ...
      <div v-if="selected" class="prop-root">
        <div class="prop-panel-head">
          <span class="prop-panel-head__glyph" aria-hidden="true">{{ selectedText ? 'A' : '▣' }}</span>
          <h2 class="prop-panel-head__title">{{ selectedText ? 'Text' : 'Image' }}</h2>
        </div>

        <template v-if="selectedText">
          <details class="prop-acc" open>
            <summary class="prop-acc-summary">Typography</summary>
            <div class="prop-acc-body">
              <label class="prop-field">
                <span class="prop-label">Content</span>
                <textarea
                  v-model="selectedText.content"
                  class="textarea prop-textarea"
                  rows="4"
                  placeholder="Type your text…"
                />
              </label>
              <div class="prop-inline-row">
                <label class="prop-field prop-field--grow">
                  <span class="prop-label">Font</span>
                  <select v-model="selectedText.fontFamily" class="select font-select prop-select">
                    <option
                ...
                <label class="prop-field prop-field--color">
                  <span class="prop-label sr-only">Color</span>
                  <input v-model="selectedText.color" type="color" class="color-input prop-color-swatch" />
                </label>
              </div>
              <div class="prop-inline-row prop-inline-row--tight">
                <label class="prop-field prop-field--grow">
                ...
                <span class="prop-mm-readout">≈ {{ fontSizePxToMmApprox(selectedText.fontSize) }} mm</span>
              </div>
              <label class="prop-field">
                <span class="prop-label">Line height</span>
                <input
                  v-model.number="selectedText.lineHeight"
                  type="number"
                  min="0.8"
                  max="3"
                  step="0.05"
              ...
              <div class="prop-toggle-row">
                <button
                  ...
                  :class="{ 'prop-toggle-btn--on': selectedText.fontWeight === '700' }"
                  title="Bold"
                  :aria-pressed="selectedText.fontWeight === '700'"
                  @click="selectedText.fontWeight = selectedText.fontWeight === '700' ? '400' : '700'"
                >
                  B
                </button>
                <button
                  type="button"
                  class="prop-toggle-btn prop-toggle-btn--italic"
                  :class="{ 'prop-toggle-btn--on': selectedText.fontStyle === 'italic' }"
                  title="Italic"
                  :aria-pressed="selectedText.fontStyle === 'italic'"
                  @click="selectedText.fontStyle = selectedText.fontStyle === 'italic' ? 'normal' : 'italic'"
                >
                  I
                </button>
                <button
                  type="button"
                  class="prop-toggle-btn prop-toggle-btn--underline"
                  :class="{ 'prop-toggle-btn--on': selectedText.textDecoration === 'underline' }"
                  title="Underline"
                  :aria-pressed="selectedText.textDecoration === 'underline'"
                  @click="
              ...
              <div class="prop-text-align-row">
                <button
                  type="button"
                  class="prop-align-mini"
                  :class="{ 'prop-align-mini--active': selectedText.textAlign === 'left' }"
                  aria-label="Align left"
                  @click="selectedText.textAlign = 'left'"
                >
                  ⟸
                </button>
                <button
                  type="button"
                  class="prop-align-mini"
                  :class="{ 'prop-align-mini--active': selectedText.textAlign === 'center' }"
                  aria-label="Align center"
                  @click="selectedText.textAlign = 'center'"
                >
                  ≡
                </button>
                <button
                  type="button"
                  class="prop-align-mini"
                  :class="{ 'prop-align-mini--active': selectedText.textAlign === 'right' }"
                  aria-label="Align right"
                  @click="selectedText.textAlign = 'right'"
                >
                  ⟹
                </button>
              </div>
        ...
        <template v-else-if="selectedImage">
          <details class="prop-acc" open>
            <summary class="prop-acc-summary">Image</summary>
            <div class="prop-acc-body">
              <div class="thumb-preview prop-thumb-preview">
              ...
              <label class="prop-field">
                <span class="prop-label">Brightness — {{ selectedImage.brightness }}%</span>
        ...
        <details class="prop-acc" open>
          <summary class="prop-acc-summary">Position</summary>
          <div class="prop-acc-body">
            <div class="prop-mm-grid">
              <label class="prop-field">
                <span class="prop-label">Width (mm)</span>
                <input
                  class="prop-num"
                  type="number"
                  step="0.1"
                  min="1"
                  :value="pctToMmW(selected.widthPct)"
                  @change="onMmWidthInput(($event.target as HTMLInputElement).value)"
                />
              </label>
              <button
                type="button"
                class="prop-lock-btn"
                :class="{ 'prop-lock-btn--on': aspectRatioLock }"
                :title="aspectRatioLock ? 'Unlock aspect ratio' : 'Lock aspect ratio'"
                :aria-pressed="aspectRatioLock"
                aria-label="Aspect ratio lock"
                @click="toggleAspectRatioLock"
              >
                <span aria-hidden="true">{{ aspectRatioLock ? '●' : '○' }}</span>
              </button>
              <label class="prop-field">
                ...
                <input
                  ...
                  step="0.1"
                  min="1"
                  :value="pctToMmH(selected.heightPct)"
                  @change="onMmHeightInput(($event.target as HTMLInputElement).value)"
                />
              </label>
              <label class="prop-field prop-mm-grid__full">
                <span class="prop-label">Left offset (mm)</span>
                <input
                  class="prop-num"
                  type="number"
                  step="0.1"
                  min="0"
                  :value="pctToMmW(selected.x)"
                  @change="onMmLeftInput(($event.target as HTMLInputElement).value)"
                />
              </label>
              <label class="prop-field prop-mm-grid__full">
                <span class="prop-label">Top offset (mm)</span>
                <input
                  class="prop-num"
                  type="number"
                  step="0.1"
                  min="0"
                  :value="pctToMmH(selected.y)"
                  @change="onMmTopInput(($event.target as HTMLInputElement).value)"
                />
              </label>
              <label class="prop-field prop-mm-grid__full">
                <span class="prop-label">Visibility — {{ Math.round((selectedImage?.opacity ?? 1) * 100) }}%</span>
                <input
                  v-if="selectedImage"
                  ...
                  class="range"
                />
                <span v-else class="prop-muted">Fixed for text (adjust via color).</span>
              </label>
            </div>
          </div>
        </details>
        ...
        <details class="prop-acc" open>
          <summary class="prop-acc-summary">Alignment &amp; fit</summary>
          <div class="prop-acc-body">
            <div class="prop-static-guide">
              <span class="prop-static-guide__label">Guide</span>
              <span class="prop-static-guide__value">Trim line</span>
            </div>
            <div class="prop-label">Align on card</div>
            <div class="prop-matrix">
              <template v-for="ri in [0, 1, 2]" :key="'r' + ri">
                <template v-for="ci in [0, 1, 2]" :key="'c' + ri + '-' + ci">
                  <button
                    type="button"
                    class="prop-matrix-cell"
                    :aria-label="`Align ${['top', 'middle', 'bottom'][ri]} ${['left', 'center', 'right'][ci]}`"
                    @click="alignSelectedGrid(ri, ci)"
                  />
                </template>
              </template>
            </div>
            <template v-if="selectedImage">
              <div class="prop-label">Fit mode</div>
              <div class="prop-fit-row">
                <button
                  type="button"
                  class="prop-fit-btn"
                  :class="{ 'prop-fit-btn--active': selectedImage.objectFit === 'cover' }"
                  @click="selectedImage.objectFit = 'cover'"
                >
                  Cover
                </button>
                <button
                  type="button"
                  class="prop-fit-btn"
                  :class="{ 'prop-fit-btn--active': selectedImage.objectFit === 'contain' }"
                  @click="selectedImage.objectFit = 'contain'"
                >
                  Contain
                </button>
                <button
                  type="button"
                  class="prop-fit-btn"
                  :class="{ 'prop-fit-btn--active': selectedImage.objectFit === 'fill' }"
                  @click="selectedImage.objectFit = 'fill'"
                >
                  Fill
                </button>
              </div>
        ...
        <details class="prop-acc" open>
          <summary class="prop-acc-summary">Layers</summary>
          <div class="prop-acc-body">
            <div class="prop-layer-bar">
              <button type="button" class="prop-layer-icon" title="Send to back" aria-label="Send to back" @click="moveLayerToBack">
                <span class="prop-layer-glyph" aria-hidden="true">⭳</span>
              </button>
              <button
                type="button"
                class="prop-layer-icon"
                title="Send backward"
                aria-label="Send backward"
                @click="moveLayerBackward"
              >
                <span class="prop-layer-glyph" aria-hidden="true">↓</span>
              </button>
              <button
                ...
                @click="moveLayerForward"
              >
                <span class="prop-layer-glyph" aria-hidden="true">↑</span>
              </button>
              <button type="button" class="prop-layer-icon" title="Bring to front" aria-label="Bring to front" @click="moveLayerToFront">
                <span class="prop-layer-glyph" aria-hidden="true">⭱</span>
              </button>
            </div>
          </div>
        ...
        <div class="prop-footer-bar">
          <button
            v-if="selectedImage"
            type="button"
            class="prop-footer-btn prop-footer-btn--icon"
          ...
          <button type="button" class="prop-footer-btn prop-footer-btn--grow" @click="duplicateSelected">
            Duplicate
          </button>
          <button type="button" class="prop-footer-btn prop-footer-btn--danger" @click="removeSelected">Delete</button>
        </div>
      </div>
...
.prop-acc-summary {
  ...
  color: #4b5563;
  background: linear-gradient(180deg, #f3f4f6 0%, #e8eaef 100%);
...
.prop-acc-summary::-webkit-details-marker {
  display: none;
}

.prop-acc-summary::after {
  content: '▾';
  font-size: 0.55rem;
  opacity: 0.65;
...
.prop-acc[open] > .prop-acc-summary::after {
...
.prop-toggle-btn--italic {
  font-style: italic;
  font-family: Georgia, serif;
}

.prop-toggle-btn--underline {
...
.prop-align-mini:hover {
  border-color: #9ca3af;
}

.prop-align-mini--active {
  border-color: #2563eb;
  background: #eff6ff;
  color: #1d4ed8;
}
...
.prop-static-guide__label {
  color: #6b7280;
  font-weight: 500;
}

.prop-static-guide__value {
  font-weight: 600;
  color: #2563eb;
}
...
.prop-footer-btn--grow {
  flex: 1;
}

.prop-footer-btn--icon {
  flex-shrink: 0;
  width: 2.5rem;
  padding-left: 0;
  padding-right: 0;
...
.prop-footer-btn:hover {
  border-color: #9ca3af;
  background: #f9fafb;
}

.prop-footer-btn--danger {
  border-color: #fecaca;
  color: #b91c1c;
  background: #fff;
}

.prop-footer-btn--danger:hover {
  background: #fef2f2;
}
